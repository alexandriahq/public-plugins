...
Credentials stay on disk after `multiplayer-mcp login`. Never put a bearer token, WorkOS session, or user PAT in plugin files or MCP `headers`.
...
- Root (service token without a user header) is the admin class. Prefer a user PAT (`mp_…`) for Codex / Claude Code.
